...
.ConanModules_{config} + .ConanModule_{name}_{config}
"""
...
class FASTBuildModule(object):
    ...
    def build_conan_module(self, deps):
        ...
        debug_properties = "\n    ".join(debug_lines)
        self.append(conan_fastbuild_module.format(name=self.name, config="Debug", properties=debug_properties))

        # Release properties
        if deps.release:
            release_lines += self.build_property_group(deps.release)
        release_properties = "\n    ".join(release_lines)
        self.append(conan_fastbuild_module.format(name=self.name, config="Release", properties=release_properties))
